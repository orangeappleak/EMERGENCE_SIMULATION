...
type PixiWorldProps = {
  ...
  routeOverlay: "none" | "all" | "selected";
  onSelectCitizen: (citizenId: string) => void;
  onSelectBuilding: (buildingId: string) => void;
};

export function PixiWorld({ sim, selectedCitizenId, followSelected, routeOverlay, onSelectCitizen, onSelectBuilding }: PixiWorldProps) {
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      try {
        ...
        world.update(sim, selectedCitizenId, followSelected, routeOverlay);
      } catch (error) {
        if (!cancelled) {
  ...
  useEffect(() => {
    worldRef.current?.update(sim, selectedCitizenId, followSelected, routeOverlay);
  }, [sim, selectedCitizenId, followSelected, routeOverlay, sim.day, sim.minute, sim.totalConversations, sim.weather.kind]);
